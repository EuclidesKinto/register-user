    ...
    <div class="container mx-auto">
      <div class="w-full ">
        ...
        <div class="px-4 overflow-x-auto bg-white shadow py-7 md:px-8 xl:px-10 2xl:overflow-x-hidden">
          <table v-if="users.length > 0" class="w-full whitespace-nowrap">
            <thead>
            <tr class="w-full h-20 text-sm leading-none text-gray-600">
              <th class="pl-4 italic font-bold text-left">#</th>
              <th class="italic font-bold text-left  ">Nome</th>
              <th class="italic font-bold text-left  ">Email</th>
              <th class="italic font-bold text-left ">Documento</th>
          ...
          <p v-else class="text-center">Ainda não temos usuários Cadastrados</p>
        </div>
      </div>
    </div>
  </div>
...
const usersLength = computed(() => store.getters['getTotalUsers']);
